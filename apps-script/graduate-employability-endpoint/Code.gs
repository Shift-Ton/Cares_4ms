/**
 * =====================================================================
 * Graduate Employability Survey — Google Apps Script Web App
 * =====================================================================
 * Deploy this script bound to the Graduate Employability Form's
 * Google Sheet (the sheet that stores the Form's responses).
 *
 * Deployment steps
 * ----------------
 *   1. Open the Google Sheet that receives the Graduate Employability
 *      form responses.
 *   2. Extensions → Apps Script.
 *   3. Paste this file, save, then Deploy → New deployment → Web app:
 *          - Execute as:  Me
 *          - Who has access: Anyone
 *      Copy the Web App URL.
 *   4. In your dashboard (http://127.0.0.1:5500/index.html) open
 *      Data Sources (the plug icon), paste the URL into
 *      "Graduate Employability Survey" and Save.
 *
 * Front-end contract
 * ------------------
 *   GET  ?action=getData&sheet=graduateEmployability
 *   →    { success: true, data: [ { …record… }, … ], rowCount: N }
 *
 * Each record is normalized so it plugs directly into the dashboard's
 * generic renderer. Field keys match the column keys defined in
 * script.js under MODULES['graduate-employability'].
 * =====================================================================
 */

const GE_VERSION = '2.1';
const GE_ENDPOINT_KEY = 'graduate-employability';

const GE_CONFIG = {
  RESPONSE_SHEET_NAME: 'Form Responses 1',

  // Additive yearbook-claim settings. These do not change the existing
  // Graduate Employability Survey endpoints or response-sheet structure.
  CLAIM_LOG_SHEET_NAME: 'Claimed Alumni Logbook',
  CLAIM_LOG_HEADERS: [
    'Claimed Date',
    'Source Row',
    'Last Name',
    'First Name',
    'Middle Name',
    'Suffix',
    'Full Name',
    'Batch',
    'Campus/College',
    'Degree Program'
  ],
  FALLBACK_SEARCH_TOKEN_TTL_SECONDS: 600,

  // Every alias below is matched case-insensitively and with
  // non-alphanumeric characters stripped, so slight header wording
  // differences in the live Form won't break the lookup.
  HEADER_ALIASES: {
    timestamp:          ['timestamp', 'submission time', 'submitted at'],
    email:              ['email address', 'email', 'e-mail address', 'e-mail'],

    lastName:           ['last name', 'surname', 'family name'],
    firstName:          ['first name', 'given name'],
    middleName:         ['middle name', 'middle initial', 'm.i.', 'mi'],
    suffix:             ['suffix (ex: sr., jr., iii, etc.):', 'suffix', 'name suffix'],

    birthdate:          ['birthdate', 'date of birth', 'birthday'],
    civilStatus:        ['civil status', 'civil'],
    sexAtBirth:         ['sex assigned at birth', 'sex at birth', 'sex'],
    genderIdentity:     ['gender identity', 'gender'],

    mobile:             ['mobile number', 'contact number', 'contact', 'mobile', 'cellphone', 'phone'],
    facebook:           ['facebook account link', 'facebook account', 'facebook', 'fb link', 'fb'],

    barangay:           ['barangay'],
    municipality:       ['municipality', 'city/municipality', 'town'],
    province:           ['province/city', 'province / city', 'province', 'city'],
    country:            ['country'],

    isIp:               ['are you member of indigenous peoples (ip) group?', 'are you a member of ip group', 'ip group'],
    isPwd:              ['are you a person with disability?', 'person with disability', 'pwd'],
    pwdType:            ['please indicate your type of disability:', 'type of disability', 'disability type', 'please indicate your type of disability'],
    ipAffiliation:      ['please indicate your indigenous people (ip) affiliation:', 'ip affiliation', 'indigenous affiliation', 'please indicate your indigenous people (ip) affiliation'],

    degree:             [
      'what degree program did you complete at romblon state university?',
      'what degree program did you complete at romblon state university',
      'what degree did you complete at romblon state university?',
      'what degree did you complete at rsu?',
      'degree program',
      'course completed',
      'degree'
    ],
    campusCollege:      ['campus/college', 'campus / college', 'campus', 'college'],
    yearGraduated:      ['year graduated', 'batch', 'graduation year'],
    firstGenCollege:    ['are you the first person in your family to graduate from college?', 'first person in your family to graduate', 'first gen college', '1st gen'],

    pursuedFurther:     ['did you pursue further studies after graduation?', 'pursued further studies'],
    furtherStudyLevel:  ['what level of further study did you pursue?', 'level of further study'],
    furtherProgram:     ['please specify the course, program, or degree.', 'course program or degree', 'further studies program'],
    furtherSchool:      ['name of institution / school', 'name of institution/school', 'institution / school', 'school'],
    furtherLocation:    ['location of institution / school', 'location of institution/school', 'school location'],
    furtherStart:       ['date started'],
    furtherEnd:         ['date completed'],

    presentlyEmployed:  ['are you presently employed?', 'presently employed', 'employed'],
    reasonsNotEmployed: ['what are the reasons why you are not currently employed?', 'reasons not currently employed'],
    lookingForJob:      ['are you currently looking for a job?', 'currently looking for a job', 'job ready'],
    supportNeeded:      ['what kind of support would help you most in finding employment?', 'support to find employment'],

    position:           ['what is your current position?', 'current position', 'position'],
    employer:           ['name of organization / employer', 'name of organization/employer', 'organization', 'employer'],
    employerLocation:   ['location of organization / employer', 'location of organization/employer', 'employer location'],

    sector:             ['sector of employment', 'employment sector'],
    employmentStatus:   ['present employment status', 'employment status'],
    industry:           ["employer's industry / line of business", "employer’s industry / line of business", 'employer industry', 'industry / line of business'],
    jobLevel:           ['current or present job level position', 'job level'],
    reasonsStaying:     ['what were your reasons for staying in your current job?', 'reasons for staying'],
    monthlySalary:      ['what is your current monthly salary in your present employment?', 'current monthly salary'],
    isFirstJob:         ['is this your first job after college?', 'first job after college'],
    firstJobRelated:    ['is your first job related to the degree you completed in college?', 'first job related to degree'],
    reasonsAccepting:   ['what were your reasons for accepting your first job?', 'reasons for accepting first job'],
    howFoundFirst:      ['how did you find your first job?', 'how found first job'],
    timeToFirst:        ['how long did it take you to get your first job after graduation?', 'time to first job'],
    firstJobIncome:     ['what was your initial gross monthly income in your first job after college?', 'first job income'],
    reasonsChangeJob:   ['if you have previously changed jobs, what were your reasons for the change?', 'reasons for changing jobs'],

    inBusiness:         ['are you currently engaged in business or entrepreneurship?', 'engaged in business', 'business'],
    businessName:       ['business / company name', 'business/company name', 'company name'],
    businessRole:       ['what is your role in the business?', 'role in business', 'business role'],
    businessType:       ['what is the nature or industry of your business?', 'nature of business', 'industry of business', 'business type'],
    businessLocation:   ['where is your business located?', 'business location', 'company address'],
    businessStart:      ['when did you start your business?', 'business start date']
  }
};

// ==============================================================
// PUBLIC ENTRY POINT
// ==============================================================
function doGet(e) {
  const params = (e && e.parameter) || {};
  const action = params.action || 'getData';
  const callback = params.callback;
  const isMobileBridge = action === 'mobileBridgePing' || action === 'mobileBridgeCheckName';
  let payload;

  try {
    if (action === 'ping' || action === 'mobileBridgePing') {
      payload = { success: true, message: 'pong', endpoint: GE_ENDPOINT_KEY, service: 'graduateEmployability', version: GE_VERSION, timestamp: new Date().toISOString() };
    } else if (action === 'getData') {
      payload = handleGetData_(params);
    } else if (action === 'debugHeaders') {
      payload = handleDebugHeaders_();
    } else if (action === 'checkName' || action === 'mobileBridgeCheckName') {
      // Existing additive endpoint used by Yearbook Quest.
      payload = handleCheckName_(params);
    } else if (action === 'lookup') {
      // New additive endpoint for the alumni yearbook lookup interface.
      payload = handleYearbookLookup_(params);
    } else if (action === 'claim') {
      // Records a selected source-sheet row in a separate claim logbook.
      payload = handleYearbookClaim_(params);
    } else if (action === 'logbook_list') {
      // Returns every claimed record, newest first.
      payload = handleYearbookLogbookList_(params);
    } else if (action === 'getEndpoints') {
      // Kept for compatibility with the dashboard's global-endpoints probe.
      payload = {
        success: true,
        endpoints: {
          yearbookLookup: true,
          yearbookClaim: true,
          yearbookLogbook: true
        }
      };
    } else {
      throw new Error('Unsupported action: ' + action);
    }
  } catch (err) {
    var errorMessage = err && err.message ? err.message : 'Unexpected server error.';
    payload = {
      success: false,
      ok: false,
      message: errorMessage,
      error: errorMessage,
      data: [],
      results: []
    };
  }

  // Installed mobile PWAs can be stricter with redirected JSONP resources.
  // This optional iframe bridge posts the result back to the installed app
  // and leaves all existing JSON/JSONP actions unchanged.
  if (isMobileBridge) return buildMobileBridgeOutput_(payload, params);
  return buildOutput_(payload, callback);
}

// ==============================================================
// CORE HANDLER
// ==============================================================
function handleGetData_(params) {
  const sheet = getResponseSheet_();
  const dataRange = sheet.getDataRange();
  const values = dataRange.getValues();
  const displayValues = dataRange.getDisplayValues();

  if (!values || values.length < 2) {
    return { success: true, data: [], rowCount: 0 };
  }

  const headerRow = values[0];
  const headerMap = buildHeaderMap_(headerRow);

  const records = [];
  for (var i = 1; i < values.length; i++) {
    var rawRow = values[i];
    var dispRow = displayValues[i];

    // Skip completely blank rows.
    if (isBlankRow_(rawRow)) continue;

    var record = buildRecord_(rawRow, dispRow, headerMap, i + 1); // i+1 = 1-based sheet row
    records.push(record);
  }

  return {
    success: true,
    data: records,
    rowCount: records.length
  };
}

// ==============================================================
// ADDITIVE YEARBOOK QUEST NAME CHECK
// ==============================================================
/**
 * Checks only the submitted first name and last name against the Form
 * Responses sheet. This endpoint does not modify any sheet data and does
 * not change the existing getData response used by the dashboard.
 *
 * Request:
 *   GET ?action=checkName&firstName=Jose&lastName=Rizal
 *
 * Response:
 *   { success: true, found: true|false, matchedName: "RIZAL, Jose" }
 */
function handleCheckName_(params) {
  var requestedFirstName = String(params.firstName || '').trim();
  var requestedLastName = String(params.lastName || '').trim();

  if (!requestedFirstName || !requestedLastName) {
    throw new Error('Both firstName and lastName are required.');
  }

  var sheet = getResponseSheet_();
  var dataRange = sheet.getDataRange();
  var values = dataRange.getDisplayValues();

  if (!values || values.length < 2) {
    return {
      success: true,
      found: false,
      firstName: requestedFirstName,
      lastName: requestedLastName,
      matchedName: ''
    };
  }

  var headerMap = buildHeaderMap_(values[0]);
  var firstNameIndex = headerMap.firstName;
  var lastNameIndex = headerMap.lastName;

  if (firstNameIndex == null || firstNameIndex < 0) {
    throw new Error('First name column was not found in the response sheet.');
  }
  if (lastNameIndex == null || lastNameIndex < 0) {
    throw new Error('Last name column was not found in the response sheet.');
  }

  var requestedFirstKey = normalizePersonName_(requestedFirstName);
  var requestedLastKey = normalizePersonName_(requestedLastName);

  for (var i = 1; i < values.length; i++) {
    var row = values[i];
    if (isBlankRow_(row)) continue;

    var rowFirstName = String(row[firstNameIndex] || '').trim();
    var rowLastName = String(row[lastNameIndex] || '').trim();

    if (
      normalizePersonName_(rowFirstName) === requestedFirstKey &&
      normalizePersonName_(rowLastName) === requestedLastKey
    ) {
      return {
        success: true,
        found: true,
        firstName: requestedFirstName,
        lastName: requestedLastName,
        matchedName: composeFullName_(rowLastName, rowFirstName, '', ''),
        sheetRow: i + 1
      };
    }
  }

  return {
    success: true,
    found: false,
    firstName: requestedFirstName,
    lastName: requestedLastName,
    matchedName: ''
  };
}

/**
 * Name-only normalization for matching. It is case-insensitive and ignores
 * spaces, punctuation, apostrophes, hyphens, and common accent marks.
 */
function normalizePersonName_(value) {
  var text = String(value || '').toLowerCase().trim();

  // V8 Apps Script supports String.normalize. Keep a safe fallback for
  // older runtimes or unusual values.
  try {
    text = text.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  } catch (ignore) {}

  return text.replace(/[^a-z0-9]/g, '');
}

// ==============================================================
// RECORD BUILDER
// ==============================================================
function buildRecord_(rawRow, dispRow, headerMap, sheetRowNumber) {
  function pick(key) {
    var idx = headerMap[key];
    if (idx == null || idx < 0) return '';
    var disp = dispRow[idx];
    var raw = rawRow[idx];
    if (disp !== '' && disp != null) return disp;
    if (raw instanceof Date) return Utilities.formatDate(raw, Session.getScriptTimeZone(), 'yyyy-MM-dd');
    return raw == null ? '' : String(raw);
  }

  var lastName   = pick('lastName');
  var firstName  = pick('firstName');
  var middleName = pick('middleName');
  var suffix     = pick('suffix');

  var barangay     = pick('barangay');
  var municipality = pick('municipality');
  var province     = pick('province');

  var record = {
    // Row identity — the dashboard reads this to show the true Google
    // Sheet row number and to key inline edits.
    id: 'row_' + sheetRowNumber,
    sheetRow: sheetRowNumber,

    // Meta
    timestamp: pick('timestamp'),
    email:     pick('email'),

    // Name parts (kept for downstream use) + composed full name.
    lastName:   lastName,
    firstName:  firstName,
    middleName: middleName,
    suffix:     suffix,
    fullName:   composeFullName_(lastName, firstName, suffix, middleName),

    // Demographics
    birthdate:       pick('birthdate'),
    civilStatus:     pick('civilStatus'),
    sexAtBirth:      pick('sexAtBirth'),
    genderIdentity:  pick('genderIdentity'),
    mobile:          pick('mobile'),
    facebook:        pick('facebook'),

    // Address (components + composed).
    barangay:     barangay,
    municipality: municipality,
    province:     province,
    address:      composeAddress_(barangay, municipality, province),

    // PWD & IP
    pwdType:       pick('pwdType'),
    ipAffiliation: pick('ipAffiliation'),

    // Academic
    degree:          pick('degree'),
    campusCollege:   pick('campusCollege'),
    yearGraduated:   pick('yearGraduated'),
    firstGenCollege: pick('firstGenCollege'),

    // Further studies (only the four displayed fields).
    furtherStudyLevel: pick('furtherStudyLevel'),
    furtherProgram:    pick('furtherProgram'),
    furtherSchool:     pick('furtherSchool'),
    furtherLocation:   pick('furtherLocation'),

    // Employment (only displayed fields).
    lookingForJob:    pick('lookingForJob'),
    position:         pick('position'),
    employer:         pick('employer'),
    employerLocation: pick('employerLocation'),
    industry:         pick('industry'),

    // Business (displayed fields).
    inBusiness:       pick('inBusiness'),
    businessName:     pick('businessName'),
    businessRole:     pick('businessRole'),
    businessType:     pick('businessType'),
    businessLocation: pick('businessLocation')
  };

  return record;
}

// ==============================================================
// FORMATTING HELPERS
// ==============================================================
function composeFullName_(last, first, suffix, middle) {
  var cleanLast   = String(last   || '').trim();
  var cleanFirst  = String(first  || '').trim();
  var cleanSuffix = String(suffix || '').trim();
  var cleanMiddle = String(middle || '').trim();

  // Format:  LASTNAME, First Name Suffix Middle Name
  var right = [cleanFirst, cleanSuffix, cleanMiddle].filter(function(x){ return x; }).join(' ');
  var left  = cleanLast ? cleanLast.toUpperCase() : '';

  if (left && right) return left + ', ' + right;
  return left || right;
}

function composeAddress_(barangay, municipality, province) {
  var parts = [barangay, municipality, province]
    .map(function(v){ return String(v || '').trim(); })
    .filter(function(v){ return v; });
  return parts.join(', ');
}

// ==============================================================
// SHEET / HEADER UTILITIES
// ==============================================================
function getSourceSpreadsheet_() {
  var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  if (!spreadsheet) {
    var spreadsheetId = String(PropertiesService.getScriptProperties().getProperty('SPREADSHEET_ID') || '').trim();
    if (spreadsheetId) spreadsheet = SpreadsheetApp.openById(spreadsheetId);
  }
  if (!spreadsheet) throw new Error('No spreadsheet found. Bind this script to the Graduate Employability response sheet or set SPREADSHEET_ID.');
  return spreadsheet;
}

function getResponseSheet_() {
  var spreadsheet = getSourceSpreadsheet_();
  var direct = spreadsheet.getSheetByName(GE_CONFIG.RESPONSE_SHEET_NAME);
  if (direct) return direct;

  var target = normalizeKey_(GE_CONFIG.RESPONSE_SHEET_NAME);
  var match = spreadsheet.getSheets().find(function(s){
    return normalizeKey_(s.getName()) === target;
  });
  if (match) return match;

  // Fallback — many form-response sheets are simply named "Form Responses 1".
  var sheets = spreadsheet.getSheets();
  if (sheets.length === 1) return sheets[0];

  throw new Error('Response sheet not found: ' + GE_CONFIG.RESPONSE_SHEET_NAME);
}

function handleDebugHeaders_() {
  var sheet = getResponseSheet_();
  var width = Math.min(Math.max(sheet.getLastColumn(), 1), 200);
  var headers = sheet.getRange(1, 1, 1, width).getDisplayValues()[0];
  var map = buildHeaderMap_(headers);
  var detected = {};
  Object.keys(map).forEach(function(key) {
    var index = map[key];
    detected[key] = index >= 0 ? { index: index, column: geColumnLetter_(index + 1), header: headers[index] } : null;
  });
  return { success: true, endpoint: GE_ENDPOINT_KEY, version: GE_VERSION, spreadsheet: getSourceSpreadsheet_().getName(), sheet: sheet.getName(), detected: detected, headers: headers };
}

function geColumnLetter_(number) {
  var n = Number(number) || 0;
  var text = '';
  while (n > 0) {
    var remainder = (n - 1) % 26;
    text = String.fromCharCode(65 + remainder) + text;
    n = Math.floor((n - 1) / 26);
  }
  return text;
}

function buildHeaderMap_(headerRow) {
  var normalizedHeaders = headerRow.map(function(h){ return normalizeKey_(h); });
  var map = {};

  Object.keys(GE_CONFIG.HEADER_ALIASES).forEach(function(key) {
    var aliases = GE_CONFIG.HEADER_ALIASES[key].map(normalizeKey_);
    var foundIndex = -1;
    for (var i = 0; i < normalizedHeaders.length; i++) {
      if (aliases.indexOf(normalizedHeaders[i]) >= 0) { foundIndex = i; break; }
    }
    // Substring fallback — some Form question labels are extremely long.
    if (foundIndex < 0) {
      for (var j = 0; j < normalizedHeaders.length; j++) {
        var nh = normalizedHeaders[j];
        for (var k = 0; k < aliases.length; k++) {
          if (nh && aliases[k] && nh.indexOf(aliases[k]) === 0) { foundIndex = j; break; }
        }
        if (foundIndex >= 0) break;
      }
    }
    map[key] = foundIndex;
  });

  return map;
}

function normalizeKey_(value) {
  return String(value || '')
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9]/g, '');
}

function isBlankRow_(row) {
  for (var i = 0; i < row.length; i++) {
    if (row[i] !== '' && row[i] != null) return false;
  }
  return true;
}

// ==============================================================
// ADDITIVE ALUMNI YEARBOOK LOOKUP / CLAIM LOGBOOK
// ==============================================================
/**
 * Search flow used by the alumni yearbook claim lookup page.
 *
 * Full-name search must be performed first. When it returns no records,
 * the response includes a temporary fallback token. First-name-only or
 * last-name-only searches require that token, preventing partial searches
 * from being used before the accurate full-name search.
 */
function handleYearbookLookup_(params) {
  var mode = String(params.searchMode || params.matchField || 'full').toLowerCase();
  if (['full', 'first', 'last'].indexOf(mode) < 0) mode = 'full';

  var requestedFirstName = String(params.firstName || '').trim();
  var requestedLastName = String(params.lastName || '').trim();

  if (mode === 'full' && (!requestedFirstName || !requestedLastName)) {
    throw new Error('Please provide both First Name and Last Name.');
  }
  if (mode === 'first' && !requestedFirstName) {
    throw new Error('Please provide a First Name for the first-name-only search.');
  }
  if (mode === 'last' && !requestedLastName) {
    throw new Error('Please provide a Last Name for the last-name-only search.');
  }

  var firstKey = normalizePersonName_(requestedFirstName);
  var lastKey = normalizePersonName_(requestedLastName);

  if (mode !== 'full') {
    validateFallbackSearchToken_(String(params.fallbackToken || ''), firstKey, lastKey);
  }

  var sheet = getResponseSheet_();
  var dataRange = sheet.getDataRange();
  var rawValues = dataRange.getValues();
  var displayValues = dataRange.getDisplayValues();

  if (!rawValues || rawValues.length < 2) {
    return buildLookupResponse_([], mode, requestedFirstName, requestedLastName);
  }

  var headerMap = buildHeaderMap_(rawValues[0]);
  ensureLookupNameColumns_(headerMap);

  var claimedByRow = getClaimedRecordMap_();
  var results = [];

  for (var i = 1; i < rawValues.length; i++) {
    var rawRow = rawValues[i];
    var displayRow = displayValues[i];
    if (isBlankRow_(rawRow)) continue;

    var rowFirstName = getDisplayOrRawCell_(rawRow, displayRow, headerMap.firstName);
    var rowLastName = getDisplayOrRawCell_(rawRow, displayRow, headerMap.lastName);
    var rowFirstKey = normalizePersonName_(rowFirstName);
    var rowLastKey = normalizePersonName_(rowLastName);

    var isMatch =
      mode === 'first'
        ? rowFirstKey === firstKey
        : mode === 'last'
          ? rowLastKey === lastKey
          : rowFirstKey === firstKey && rowLastKey === lastKey;

    if (!isMatch) continue;

    var sourceRow = i + 1;
    var claimedInfo = claimedByRow[String(sourceRow)] || null;
    results.push(buildYearbookLookupRecord_(rawRow, displayRow, headerMap, sourceRow, claimedInfo));
  }

  // Keep source-sheet order stable, with newer form responses first when
  // multiple partial-search results are found.
  results.sort(function(a, b) {
    return Number(b.rowNumber || 0) - Number(a.rowNumber || 0);
  });

  return buildLookupResponse_(results, mode, requestedFirstName, requestedLastName);
}

function buildLookupResponse_(results, mode, requestedFirstName, requestedLastName) {
  var response = {
    success: true,
    ok: true,
    searchMode: mode,
    resultCount: results.length,
    results: results
  };

  if (mode === 'full' && results.length === 0) {
    response.fallbackToken = createFallbackSearchToken_(requestedFirstName, requestedLastName);
    response.partialSearchAllowed = true;
  }

  return response;
}

function ensureLookupNameColumns_(headerMap) {
  if (headerMap.firstName == null || headerMap.firstName < 0) {
    throw new Error('First name column was not found in the response sheet.');
  }
  if (headerMap.lastName == null || headerMap.lastName < 0) {
    throw new Error('Last name column was not found in the response sheet.');
  }
}

function buildYearbookLookupRecord_(rawRow, displayRow, headerMap, sourceRow, claimedInfo) {
  function pick(key) {
    return getDisplayOrRawCell_(rawRow, displayRow, headerMap[key]);
  }

  var lastName = pick('lastName');
  var firstName = pick('firstName');
  var middleName = pick('middleName');
  var suffix = pick('suffix');
  var batch = pick('yearGraduated');

  return {
    rowNumber: sourceRow,
    sheetRow: sourceRow,
    timestamp: pick('timestamp'),
    firstName: firstName,
    middleName: middleName,
    lastName: lastName,
    suffix: suffix,
    fullName: composeFullName_(lastName, firstName, suffix, middleName),
    degreeProgram: pick('degree'),
    degree: pick('degree'),
    campusCollege: pick('campusCollege'),
    batch: batch,
    yearGraduated: batch,
    isClaimed: Boolean(claimedInfo),
    claimed: Boolean(claimedInfo),
    claimStatus: claimedInfo ? 'claimed' : '',
    dateClaimed: claimedInfo ? claimedInfo.dateClaimed : ''
  };
}

function createFallbackSearchToken_(firstName, lastName) {
  var token = Utilities.getUuid().replace(/-/g, '');
  var data = {
    firstKey: normalizePersonName_(firstName),
    lastKey: normalizePersonName_(lastName),
    createdAt: Date.now()
  };

  CacheService.getScriptCache().put(
    'yearbookFallback:' + token,
    JSON.stringify(data),
    GE_CONFIG.FALLBACK_SEARCH_TOKEN_TTL_SECONDS
  );

  return token;
}

function validateFallbackSearchToken_(token, firstKey, lastKey) {
  if (!token) {
    throw new Error('Run the First Name and Last Name search first before using a one-name search.');
  }

  var cached = CacheService.getScriptCache().get('yearbookFallback:' + token);
  if (!cached) {
    throw new Error('The alternative-search session expired. Search using both names again.');
  }

  var parsed;
  try {
    parsed = JSON.parse(cached);
  } catch (ignore) {
    throw new Error('The alternative-search session is invalid. Search using both names again.');
  }

  if (parsed.firstKey !== firstKey || parsed.lastKey !== lastKey) {
    throw new Error('The entered name changed. Search using both names again before using a one-name search.');
  }
}

/**
 * Adds one or more source response rows to the separate claim logbook.
 * Existing Graduate Employability data is never modified.
 */
function handleYearbookClaim_(params) {
  var rowNumbers = parseRowNumbers_(params.rowNumbers);
  if (!rowNumbers.length) {
    throw new Error('No valid alumni row was selected for claiming.');
  }

  var lock = LockService.getScriptLock();
  if (!lock.tryLock(15000)) {
    throw new Error('Another claim is being saved. Please try again in a moment.');
  }

  try {
    var sourceSheet = getResponseSheet_();
    var sourceRange = sourceSheet.getDataRange();
    var rawValues = sourceRange.getValues();
    var displayValues = sourceRange.getDisplayValues();

    if (!rawValues || rawValues.length < 2) {
      throw new Error('No alumni records are available in the response sheet.');
    }

    var headerMap = buildHeaderMap_(rawValues[0]);
    ensureLookupNameColumns_(headerMap);

    var logSheet = getClaimLogbookSheet_(true);
    var logHeaderMap = ensureClaimLogHeaders_(logSheet);
    var claimedByRow = getClaimedRecordMap_(logSheet, logHeaderMap);
    var now = new Date();
    var rowsToAppend = [];
    var savedRecords = [];
    var skippedCount = 0;

    rowNumbers.forEach(function(sourceRow) {
      if (sourceRow <= 1 || sourceRow > rawValues.length) {
        skippedCount++;
        return;
      }

      if (claimedByRow[String(sourceRow)]) {
        skippedCount++;
        return;
      }

      var rawRow = rawValues[sourceRow - 1];
      var displayRow = displayValues[sourceRow - 1];
      if (isBlankRow_(rawRow)) {
        skippedCount++;
        return;
      }

      var claimRecord = buildClaimRecordFromSource_(rawRow, displayRow, headerMap, sourceRow, now);
      rowsToAppend.push(buildClaimLogRow_(claimRecord, logHeaderMap, logSheet.getLastColumn()));
      savedRecords.push(claimRecord);
      claimedByRow[String(sourceRow)] = claimRecord;
    });

    if (rowsToAppend.length) {
      var startRow = logSheet.getLastRow() + 1;
      logSheet.getRange(startRow, 1, rowsToAppend.length, logSheet.getLastColumn()).setValues(rowsToAppend);

      var claimDateColumn = logHeaderMap.claimedDate + 1;
      logSheet.getRange(startRow, claimDateColumn, rowsToAppend.length, 1)
        .setNumberFormat('mmmm d, yyyy h:mm AM/PM');
    }

    return {
      success: true,
      ok: true,
      insertedCount: rowsToAppend.length,
      skippedCount: skippedCount,
      filterDate: Utilities.formatDate(now, Session.getScriptTimeZone(), 'yyyy-MM-dd'),
      results: savedRecords
    };
  } finally {
    lock.releaseLock();
  }
}

function parseRowNumbers_(value) {
  var seen = {};
  return String(value || '')
    .split(',')
    .map(function(part) { return Number(String(part).trim()); })
    .filter(function(number) {
      if (!Number.isInteger(number) || number <= 1 || seen[number]) return false;
      seen[number] = true;
      return true;
    });
}

function buildClaimRecordFromSource_(rawRow, displayRow, headerMap, sourceRow, claimedDate) {
  function pick(key) {
    return getDisplayOrRawCell_(rawRow, displayRow, headerMap[key]);
  }

  var lastName = pick('lastName');
  var firstName = pick('firstName');
  var middleName = pick('middleName');
  var suffix = pick('suffix');

  return {
    sourceRow: sourceRow,
    dateClaimed: claimedDate,
    claimedDate: claimedDate,
    lastName: lastName,
    firstName: firstName,
    middleName: middleName,
    suffix: suffix,
    fullName: composeFullName_(lastName, firstName, suffix, middleName),
    batch: pick('yearGraduated'),
    yearGraduated: pick('yearGraduated'),
    campusCollege: pick('campusCollege'),
    degreeProgram: pick('degree')
  };
}

/**
 * Returns every logbook record. No date filter is applied. Latest entries
 * are returned first so the first book spread always contains the newest 20.
 */
function handleYearbookLogbookList_(params) {
  var logSheet = getClaimLogbookSheet_(false);
  if (!logSheet || logSheet.getLastRow() < 2) {
    return {
      success: true,
      ok: true,
      results: [],
      total: 0,
      sortOrder: 'latest'
    };
  }

  var headerMap = ensureClaimLogHeaders_(logSheet);
  var values = logSheet.getDataRange().getValues();
  var displayValues = logSheet.getDataRange().getDisplayValues();
  var results = [];

  for (var i = 1; i < values.length; i++) {
    if (isBlankRow_(values[i])) continue;
    results.push(buildClaimRecordFromLogRow_(values[i], displayValues[i], headerMap, i + 1));
  }

  results.sort(function(a, b) {
    var timeA = getDateSortValue_(a.dateClaimed);
    var timeB = getDateSortValue_(b.dateClaimed);
    if (timeB !== timeA) return timeB - timeA;
    return Number(b.logbookRow || 0) - Number(a.logbookRow || 0);
  });

  return {
    success: true,
    ok: true,
    results: results,
    total: results.length,
    sortOrder: 'latest'
  };
}

function getClaimLogbookSheet_(createIfMissing) {
  var spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = spreadsheet.getSheetByName(GE_CONFIG.CLAIM_LOG_SHEET_NAME);

  if (!sheet && !createIfMissing) return null;

  if (!sheet) {
    sheet = spreadsheet.insertSheet(GE_CONFIG.CLAIM_LOG_SHEET_NAME);
    sheet.getRange(1, 1, 1, GE_CONFIG.CLAIM_LOG_HEADERS.length)
      .setValues([GE_CONFIG.CLAIM_LOG_HEADERS]);
    sheet.setFrozenRows(1);
    sheet.getRange(1, 1, 1, GE_CONFIG.CLAIM_LOG_HEADERS.length)
      .setFontWeight('bold')
      .setBackground('#166534')
      .setFontColor('#ffffff');
  }

  ensureClaimLogHeaders_(sheet);
  return sheet;
}

function ensureClaimLogHeaders_(sheet) {
  if (sheet.getMaxColumns() < GE_CONFIG.CLAIM_LOG_HEADERS.length) {
    sheet.insertColumnsAfter(
      sheet.getMaxColumns(),
      GE_CONFIG.CLAIM_LOG_HEADERS.length - sheet.getMaxColumns()
    );
  }

  if (sheet.getLastRow() === 0) {
    sheet.getRange(1, 1, 1, GE_CONFIG.CLAIM_LOG_HEADERS.length)
      .setValues([GE_CONFIG.CLAIM_LOG_HEADERS]);
  }

  var lastColumn = Math.max(sheet.getLastColumn(), GE_CONFIG.CLAIM_LOG_HEADERS.length);
  var currentHeaders = sheet.getRange(1, 1, 1, lastColumn).getDisplayValues()[0];
  var normalized = currentHeaders.map(normalizeKey_);

  GE_CONFIG.CLAIM_LOG_HEADERS.forEach(function(header) {
    var key = normalizeKey_(header);
    if (normalized.indexOf(key) >= 0) return;

    currentHeaders.push(header);
    normalized.push(key);
  });

  if (currentHeaders.length > sheet.getMaxColumns()) {
    sheet.insertColumnsAfter(sheet.getMaxColumns(), currentHeaders.length - sheet.getMaxColumns());
  }

  sheet.getRange(1, 1, 1, currentHeaders.length).setValues([currentHeaders]);
  sheet.setFrozenRows(1);

  return buildClaimLogHeaderMap_(currentHeaders);
}

function buildClaimLogHeaderMap_(headers) {
  var normalized = headers.map(normalizeKey_);

  function find(aliases) {
    var keys = aliases.map(normalizeKey_);
    for (var i = 0; i < normalized.length; i++) {
      if (keys.indexOf(normalized[i]) >= 0) return i;
    }
    return -1;
  }

  return {
    claimedDate: find(['Claimed Date', 'Date Claimed', 'Claim Date']),
    sourceRow: find(['Source Row', 'Row Number', 'Sheet Row']),
    lastName: find(['Last Name', 'Surname']),
    firstName: find(['First Name', 'Given Name']),
    middleName: find(['Middle Name', 'Middle Initial']),
    suffix: find(['Suffix', 'Name Suffix']),
    fullName: find(['Full Name', 'Name']),
    batch: find(['Batch', 'Year Graduated', 'Graduation Year']),
    campusCollege: find(['Campus/College', 'Campus', 'College']),
    degreeProgram: find(['Degree Program', 'Degree', 'Course'])
  };
}

function buildClaimLogRow_(record, headerMap, columnCount) {
  var row = new Array(columnCount).fill('');

  setRowValue_(row, headerMap.claimedDate, record.dateClaimed);
  setRowValue_(row, headerMap.sourceRow, record.sourceRow);
  setRowValue_(row, headerMap.lastName, record.lastName);
  setRowValue_(row, headerMap.firstName, record.firstName);
  setRowValue_(row, headerMap.middleName, record.middleName);
  setRowValue_(row, headerMap.suffix, record.suffix);
  setRowValue_(row, headerMap.fullName, record.fullName);
  setRowValue_(row, headerMap.batch, record.batch);
  setRowValue_(row, headerMap.campusCollege, record.campusCollege);
  setRowValue_(row, headerMap.degreeProgram, record.degreeProgram);

  return row;
}

function buildClaimRecordFromLogRow_(rawRow, displayRow, headerMap, logbookRow) {
  function pick(index) {
    return getDisplayOrRawCell_(rawRow, displayRow, index);
  }

  var rawClaimDate = headerMap.claimedDate >= 0 ? rawRow[headerMap.claimedDate] : '';
  var claimedDate = serializeDateForClient_(rawClaimDate || pick(headerMap.claimedDate));

  return {
    logbookRow: logbookRow,
    sourceRow: Number(pick(headerMap.sourceRow)) || '',
    lastName: pick(headerMap.lastName),
    firstName: pick(headerMap.firstName),
    middleName: pick(headerMap.middleName),
    suffix: pick(headerMap.suffix),
    fullName: pick(headerMap.fullName),
    batch: pick(headerMap.batch),
    yearGraduated: pick(headerMap.batch),
    campusCollege: pick(headerMap.campusCollege),
    degreeProgram: pick(headerMap.degreeProgram),
    dateClaimed: claimedDate,
    claimedDate: claimedDate,
    isClaimed: true,
    claimed: true,
    claimStatus: 'claimed'
  };
}

function getClaimedRecordMap_(optionalSheet, optionalHeaderMap) {
  var sheet = optionalSheet || getClaimLogbookSheet_(false);
  var map = {};
  if (!sheet || sheet.getLastRow() < 2) return map;

  var headerMap = optionalHeaderMap || ensureClaimLogHeaders_(sheet);
  var values = sheet.getDataRange().getValues();
  var displayValues = sheet.getDataRange().getDisplayValues();

  for (var i = 1; i < values.length; i++) {
    if (isBlankRow_(values[i])) continue;
    var record = buildClaimRecordFromLogRow_(values[i], displayValues[i], headerMap, i + 1);
    if (record.sourceRow) map[String(record.sourceRow)] = record;
  }

  return map;
}

function setRowValue_(row, index, value) {
  if (index == null || index < 0 || index >= row.length) return;
  row[index] = value == null ? '' : value;
}

function getDisplayOrRawCell_(rawRow, displayRow, index) {
  if (index == null || index < 0) return '';

  var displayValue = displayRow && index < displayRow.length ? displayRow[index] : '';
  if (displayValue !== '' && displayValue != null) return String(displayValue).trim();

  var rawValue = rawRow && index < rawRow.length ? rawRow[index] : '';
  if (rawValue instanceof Date) {
    return Utilities.formatDate(rawValue, Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss');
  }
  return rawValue == null ? '' : String(rawValue).trim();
}

function serializeDateForClient_(value) {
  if (value instanceof Date && !isNaN(value.getTime())) return value.toISOString();
  if (!value) return '';

  var parsed = new Date(value);
  if (!isNaN(parsed.getTime())) return parsed.toISOString();
  return String(value);
}

function getDateSortValue_(value) {
  if (value instanceof Date && !isNaN(value.getTime())) return value.getTime();
  var parsed = new Date(value);
  return isNaN(parsed.getTime()) ? 0 : parsed.getTime();
}

// ==============================================================
// MOBILE PWA IFRAME BRIDGE
// ============================================================== 
/**
 * Returns a tiny HTML page that posts a name-check or ping result back to
 * the parent Yearbook Quest PWA. This is additive and does not alter the
 * existing JSON and JSONP endpoints.
 */
function buildMobileBridgeOutput_(payload, params) {
  var requestId = String((params && params.requestId) || '')
    .replace(/[^a-zA-Z0-9_-]/g, '')
    .slice(0, 120);

  var message = {
    source: 'yearbookQuestAppsScriptBridge',
    requestId: requestId,
    payload: payload
  };

  var serialized = JSON.stringify(message)
    .replace(/</g, '\\u003c')
    .replace(/\u2028/g, '\\u2028')
    .replace(/\u2029/g, '\\u2029');

  var html = '<!doctype html><html><head><meta charset="utf-8">' +
    '<meta name="viewport" content="width=device-width,initial-scale=1">' +
    '</head><body><script>' +
    '(function(){var message=' + serialized + ';' +
    'function send(){try{window.parent.postMessage(message,"*");}catch(e){}' +
    'try{window.top.postMessage(message,"*");}catch(e){}}' +
    'send();setTimeout(send,250);setTimeout(send,900);})();' +
    '<\\/script></body></html>';

  return HtmlService
    .createHtmlOutput(html)
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

// ==============================================================
// RESPONSE ENVELOPE
// ==============================================================
function buildOutput_(payload, callback) {
  if (callback) {
    var safe = String(callback).replace(/[^a-zA-Z0-9_$.]/g, '');
    return ContentService
      .createTextOutput(safe + '(' + JSON.stringify(payload) + ');')
      .setMimeType(ContentService.MimeType.JAVASCRIPT);
  }
  return ContentService
    .createTextOutput(JSON.stringify(payload))
    .setMimeType(ContentService.MimeType.JSON);
}
